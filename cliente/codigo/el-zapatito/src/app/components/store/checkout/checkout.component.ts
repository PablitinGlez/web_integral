                        ...
                        <div class="coupon-row">
                          ...
                          <div class="coupon-input-wrapper">
                            ...
                            <div class="input-btn-row">
                              ...
                              <button type="button" class="btn-apply-coupon" (click)="applyCoupon()">Aplicar</button>
                            </div>
                            @if (couponMessage) {
                              <p [style.color]="couponSuccess ? '#166534' : '#dc2626'" style="font-size: 0.85rem; margin: 0.25rem 0 0; font-weight: 600;">
                                {{ couponMessage }}
                              </p>
                            }
                          </div>
              ...
              <div class="summary-card">
                ...
                <div class="summary-details-list">
                  ...
                  <div class="summary-total">
                    ...
                    <span class="total-amount">{{ (cart.cartTotal() - discountAmount) | currency:'USD' }}</span>
                  </div>
                </div>
...
export class CheckoutComponent implements OnInit {
        ...
        this.loadPaypalScript(clientId).then((paypal) => {
          ...
          paypal.Buttons({
            createOrder: (data: any, actions: any) => {
              const finalAmount = (this.cart.cartTotal() - this.discountAmount).toFixed(2);
  ...
  applyCoupon() {
    ...
    this.couponService.validateCoupon(code, this.cart.cartTotal()).subscribe({
      next: (res) => {
        this.appliedCoupon = res;
        this.discountAmount = res.discount_amount;
        this.couponMessage = `¡Cupón aplicado! Descuento: -${res.discount_amount.toLocaleString('en-US', { style: 'currency', currency: 'USD' })}`;
        this.couponSuccess = true;
        this.initPaypalButtons();
      },
      error: (err) => {
        this.couponMessage = err.error?.detail || 'El cupón no es válido o ha expirado.';
        this.couponSuccess = false;
      }
    });
  }
