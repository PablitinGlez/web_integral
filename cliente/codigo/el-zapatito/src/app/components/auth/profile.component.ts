...
@Component({
                ...
                <div class="favorites-grid">
                  @for (item of favorites(); track item.id) {
                    <div class="favorite-card">
                      <img [src]="item.main_image_url || 'https://images.unsplash.com/photo-1549298916-b41d501d3772?auto=format&fit=crop&q=80&w=200'" [alt]="item.name">
                      <div class="favorite-card-info">
                        <p class="favorite-brand">{{ item.brand || 'El Zapatito' }}</p>
                        <h4>{{ item.name }}</h4>
                        <p class="favorite-price">{{ item.price | currency:'USD' }}</p>
                      </div>
                      <button class="btn-mini danger" (click)="removeFavorite(item.id)">Eliminar</button>
                    </div>
                  }
                </div>
              }
  ...
  styles: [`
    ...
    .favorites-grid { display: flex; flex-direction: column; gap: 0.9rem; }
    .favorite-card { display: flex; align-items: center; gap: 1rem; padding: 0.8rem; border: 1px solid #eee; border-radius: 10px; background: #fafafa; }
    .favorite-card img { width: 64px; height: 64px; object-fit: cover; border-radius: 10px; }
    .favorite-card-info { flex: 1; min-width: 0; }
    .favorite-card-info h4 { margin: 0.1rem 0; font-size: 0.95rem; }
    .favorite-brand { margin: 0; color: #999; font-size: 0.7rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.8px; }
    .favorite-price { margin: 0.2rem 0 0; font-weight: 700; color: #000; }
...
export class ProfileComponent implements OnInit {
  ...
  ngOnInit() {
    this.favorites.set(this.favoritesService.favorites());
  ...
  private async loadProfileData(userId: string) {
    try {
      ...
      if (token) {
        await this.addresses.loadAddresses(userId);
        this.loadMyOrders();
      } else {
        this.orders.set([]);
        this.loadingOrders.set(false);
